...
public class SettingsController {
    ...
    @Operation(summary = "ERP 자동 설정 변경 (자동전표생성, 자동ERP전송)")
    @PatchMapping("/erp/{id}/auto-settings")
    public ApiResponse<ErpConfigResponse> updateErpAutoSettings(
            @PathVariable UUID id,
            @RequestBody ErpAutoSettingsRequest request) {
        return ApiResponse.ok(erpConfigService.updateAutoSettings(id, request.autoGenerateDocument(), request.autoSendToErp()));
    }

    record ErpAutoSettingsRequest(
            Boolean autoGenerateDocument,
            Boolean autoSendToErp
    ) {}
