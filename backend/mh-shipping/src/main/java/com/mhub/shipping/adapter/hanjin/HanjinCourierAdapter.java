...
public class HanjinCourierAdapter implements CourierAdapter {
    ...
    public CourierType getCourierType() {
        return CourierType.HANJIN;
    }

    @Override
    public ReservationResult reservePickup(TenantCourierConfig config, PickupRequest req) {
        log.info("Hanjin pickup for {}", req.trackingNumber());
        return new ReservationResult(false, null, "Hanjin API not yet implemented");
    }

    @Override
    public void cancelReservation(TenantCourierConfig config, String trackingNumber) {
        log.info("Hanjin cancel {}", trackingNumber);
    }
}
