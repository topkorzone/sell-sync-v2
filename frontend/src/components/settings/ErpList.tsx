...
import { MoreHorizontal, Plus, Wifi, Pencil, Trash2, Server, Loader2, Database, FileText, FileCheck, Send } from "lucide-react";
...
export default function ErpList() {
  ...
  const [updatingAutoSettingsId, setUpdatingAutoSettingsId] = useState<string | null>(null);
  ...
  const handleToggleAutoSetting = async (
    cfg: ErpConfigResponse,
    field: "autoGenerateDocument" | "autoSendToErp",
    value: boolean
  ) => {
    setUpdatingAutoSettingsId(cfg.id);
    try {
      await api.patch<ApiResponse<ErpConfigResponse>>(
        `/api/v1/settings/erp/${cfg.id}/auto-settings`,
        { [field]: value }
      );
      toast.success(
        field === "autoGenerateDocument"
          ? `자동 전표생성이 ${value ? "활성화" : "비활성화"}되었습니다`
          : `자동 ERP전송이 ${value ? "활성화" : "비활성화"}되었습니다`
      );
      fetchConfigs();
    } catch {
      toast.error("설정 변경 중 오류가 발생했습니다");
    } finally {
      setUpdatingAutoSettingsId(null);
    }
  };
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">자동 전표생성</TableHead>
                <TableHead className="text-center">자동 ERP전송</TableHead>
                ...
                <TableRow key={cfg.id}>
                  ...
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-2">
                      <Switch
                        checked={cfg.autoGenerateDocument ?? false}
                        onCheckedChange={(checked: boolean) =>
                          handleToggleAutoSetting(cfg, "autoGenerateDocument", checked)
                        }
                        disabled={updatingAutoSettingsId === cfg.id}
                      />
                      <FileCheck className={`h-4 w-4 ${cfg.autoGenerateDocument ? "text-green-600" : "text-muted-foreground"}`} />
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-2">
                      <Switch
                        checked={cfg.autoSendToErp ?? false}
                        onCheckedChange={(checked: boolean) =>
                          handleToggleAutoSetting(cfg, "autoSendToErp", checked)
                        }
                        disabled={updatingAutoSettingsId === cfg.id || !cfg.autoGenerateDocument}
                      />
                      <Send className={`h-4 w-4 ${cfg.autoSendToErp ? "text-blue-600" : "text-muted-foreground"}`} />
                    </div>
                  </TableCell>
